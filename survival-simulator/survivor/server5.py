...
SURVIVOR_PARAMS=runs/fin4/final_config.json python -m survivor.server5
"""
import os
...
app=FastAPI(title='Survival fin5')
COUNTERS={'requests':0,'episode_requests':0,'errors':0,'episode':0,'policy_seconds':0.,'max_policy_ms':0.,'last_t':-1.,'last_score':0.}

@app.get('/')
def index():
    return {'message':'Agent endpoint running!','controller':DOC.get('controller','fin3'),'config':CONFIG,'escape':DOC.get('escape',{}),'experimental':DOC.get('experimental',False)}

@app.get('/stats')
def stats():
    return dict(COUNTERS,policy_ms_mean=1000*COUNTERS['policy_seconds']/max(1,COUNTERS['episode_requests']))
...
if __name__=='__main__':
    import uvicorn
    print(f'fin5 server config={CONFIG}; controller={DOC.get("controller","fin3")}; single process',flush=True)
