...
SURVIVOR_PARAMS=runs/fin4/final_config.json python -m survivor.server6
"""
import os
...
app=FastAPI(title='Survival fin6')
COUNTERS={'requests':0,'episode_requests':0,'errors':0,'episode':0,'policy_seconds':0.,'max_policy_ms':0.,'last_t':-1.,'last_score':0.}

@app.get('/')
def index():
    return {'message':'Agent endpoint running!','controller':DOC.get('controller','fin3'),'config':CONFIG,'escape':DOC.get('escape',{}),'experimental':DOC.get('experimental',False),'supervisor':DOC.get('supervisor',{})}

@app.get('/stats')
def stats():
    return dict(COUNTERS,options=dict(getattr(CTRL,'extra',{})),policy_ms_mean=1000*COUNTERS['policy_seconds']/max(1,COUNTERS['episode_requests']))
...
async def predict(request:Request):
    ...
    if t<COUNTERS['last_t']-1e-6:
        ...
        CTRL=load_controller(DOC,seed=0)
        COUNTERS['episode']+=1; COUNTERS['episode_requests']=0; COUNTERS['policy_seconds']=0.; COUNTERS['max_policy_ms']=0.
    ...
    COUNTERS['requests']+=1; COUNTERS['episode_requests']+=1; COUNTERS['policy_seconds']+=elapsed
...
if __name__=='__main__':
    import uvicorn
    print(f'fin6 server config={CONFIG}; controller={DOC.get("controller","fin3")}; single process',flush=True)
