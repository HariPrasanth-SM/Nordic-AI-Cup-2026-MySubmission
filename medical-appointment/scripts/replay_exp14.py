"""CPU-only calibration of a saved Exp13 trace. No model calls and no labels in inference."""
import argparse,copy,csv,json,sys,hashlib
from dataclasses import asdict
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.path.insert(0,str(ROOT))
from solution.calibration14 import calibrate,load_settings
...
def main():
    ...
    p.add_argument('--csv',type=Path,default=ROOT/'data/question_train.csv');p.add_argument('--run-name',default='exp14-cached');args=p.parse_args()
    ...
    for t in traces:
        ...
        if 'exp14_calibration' in stages:raise SystemExit('Refusing to apply calibration twice')
        required='exp13_baseline' if cfg['base_experiment']=='exp13' else 'exp12_config'
        if required not in stages or (cfg['base_experiment']=='exp12' and 'exp13_baseline' in stages):raise SystemExit('Trace does not match configured base')
    ...
    with (out/'trace.jsonl').open('w') as tf,(out/'predictions.jsonl').open('w') as pf:
        for t in traces:
            ...
            rec['elapsed_s']=None
            rec['events'] += [dict(stage='exp14_baseline',results=[asdict(r) for r in old]),dict(stage='exp14_calibration',config=cfg,changed=sum(a.span!=b.span for a,b in zip(old,new)),classification_changes=0)]
