...
def calibration_analysis(run):
    ...
    expected=len({r['conversation'] for r in rows});cal=[e for e in es if e['stage']=='exp14_calibration']
    ...
    for row in rows:
        ...
        baselines=[e['results'] for e in t.get('events',[]) if e['stage']=='exp14_baseline']
        old=as_pair(baselines[-1][idx]['span']) if baselines and idx is not None else None
        gold=(float(row['gold_start']),float(row['gold_end']));before=iou(old,gold);after=float(row['tiou'])
    ...
    (p/'exp14_execution.json').write_text(json.dumps(result,indent=2));return result
